...
//errno_t replaced by int
int initialize_tls_structure(unsigned char *raw, int size, TLSPlaintext *tls_message) {
...
void print_tls_message(TLSPlaintext *tls_message) {
	...
	printf("Fragment raw data: \n\n");



	for (i = 0; i < tls_message->length; i++) {

		printf("0x%x ", tls_message->fragment[i]); 
